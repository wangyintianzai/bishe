...
public class XinwenController extends BaseController {
    ...
    public String list() {
        ...
        List<Xinwen> list = service.selectPage(example, page, 12); // 获取当前页的行数
        // 将列表写给界面使用
        assign("list", list);
        assign("orderby", order); // 把当前排序结果写进前台
        assign("sort", sort); // 把当前排序结果写进前台
        assign("where", where); // 把当前条件写给前台
        return "xinwen_list"; // 使用视图文件：WebRoot\xinwen_list.jsp
    }
    ...
    public String index() {
        ...
        List<Xinwen> list = service.selectPage(example, page, 12);
    ...
    @RequestMapping("/xinwen_updt")
    public String updt() {
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Xinwen mmm = service.find(id);
     ...
     * @return
     */
    @RequestMapping("/xinweninsert")
    public String insert() {
        String tmp = "";
        Xinwen post = new Xinwen(); // 创建实体类
        // 设置前台提交上来的数据到实体类中
     ...
     *  前台详情
     */
    @RequestMapping("/xinwendetail")
    public String detailweb() {
        int id = Request.getInt("id");
        Xinwen map = service.find(id);
        DB.execute("UPDATE xinwen SET dianjilv=dianjilv+1 WHERE id=" + request.getParameter("id") + "");

        request.setAttribute("map", map);
        return "xinwendetail"; // 前台详情页面：WebRoot\xinwendetail.jsp
    }

    /**
     *  删除
     */
    @RequestMapping("/xinwen_delete")
    public String delete() {
        if (!checkLogin()) {
            return showError("尚未登录");
        }
        int id = Request.getInt("id"); // 根据id 删除某行数据
        Map map = Query.make("xinwen").find(id);
